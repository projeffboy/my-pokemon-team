...
export default {
  ...
  copycat: {
    ...
    onHit: function (pokemon) {
      ...
      if (
        !this.lastMove ||
        noCopycat.includes(this.lastMove.id) ||
        this.lastMove.isZ
      ) {
        return false;
      }
    ...
    effect: {
      ...
      onStart: function (target) {
        ...
        if (
          !target.lastMove ||
          target.lastMove.isZ ||
          noEncore.includes(target.lastMove.id) ||
    ...
    effect: {
      onStart: function (target, source, effect) {
        if (effect && effect.id === "zpower") {
          ...
          effect &&
          (effect.id === "transform" || effect.id === "imposter")
        ) {
  ...
  leechseed: {
    ...
    effect: {
      ...
      onResidual: function (pokemon) {
        let target =
          this.effectData.source.side.active[
  ...
  mimic: {
    ...
    onHit: function (target, source) {
      ...
      if (
        source.transformed ||
        !target.lastMove ||
        disallowedMoves.includes(target.lastMove.id) ||
        source.moves.indexOf(target.lastMove.id) >= 0 ||
        target.lastMove.isZ
      )
        return false;
