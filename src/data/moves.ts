...
const data: Record<string, any> = {
  ...
  allyswitch: {
    ...
    onHit(pokemon) {
      ...
      if (this.gameType !== "doubles" && this.gameType !== "triples")
        success = false;
  ...
  axekick: {
    ...
    onMoveFail(target, source, move) {
      this.damage(
        ...
        source,
        source,
        this.dex.conditions.get("Axe Kick"),
      );
    },
    secondary: {
      chance: 30,
  ...
  bugbite: {
    ...
    onHit(target, source, move) {
      ...
      if (source.hp && item.isBerry && target.takeItem(source)) {
        ...
        if (
          this.singleEvent("Eat", item, target.itemState, source, source, move)
        ) {
  ...
  pursuit: {
    ...
    beforeTurnCallback(pokemon, target) {
      pokemon.addVolatile(
        "pursuit",
        pokemon,
        this.dex.getActiveMove("pursuit"),
      );
    },
    onModifyMove(move, source, target) {
      if (target?.beingCalledBack || target?.switchFlag) {
    ...
    condition: {
      ...
      onFoeBeforeSwitchOut(pokemon) {
        ...
        if (
          ...
          !source.hp ||
          (source.volatiles["encore"] &&
            source.volatiles["encore"].move !== "pursuit") ||
    ...
    onTryMove(attacker, defender, move) {
      ...
      if (
        ["sunnyday", "desolateland"].includes(
          attacker.effectiveWeather(undefined, true),
        )
      ) {
        this.attrLastMove("[still]");
        this.addMove("-anim", attacker, move.name, defender);
