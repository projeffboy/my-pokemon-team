    ...
    const handleClick = (e: React.MouseEvent<HTMLElement>, i: number) => {
      ...
      const gridItems = (currentTitle: TeamStatTitle) =>
        (Object.keys(types) as PokemonType[]).map((type, i) => (
          <Grid key={i} size={2}>
            <Box sx={teamStatsStyles.typeContainer}>
              {/* Activates Popover */}
              <Box
...
const TypeDefenceTooltipInfo = ({ typeColor, type }: TeamStatsTypeTooltipProps) => (
  <>
    <p>
      <span style={{ color: `#${typeColor}` }}>{type}</span> does...
    </p>
    <ul style={teamStatsStyles.list}>
        ...
        if (pkmn) {
          ...
          return (
            ...
            <li
              ...
              style={teamStatsStyles.listItem}
            >
              <span
                style={{
                  color,
                  ...(teamStatsStyles.multiplier as React.CSSProperties),
                }}
              >
                {multiplier}x
              </span>
...
function TypeCoverageTooltipInfo({ typeColor, type }: TeamStatsTypeTooltipProps) {
                ...
                if (
                  ...
                  return (
                    ...
                    <li
                      key={move + num}
                      style={{
                        ...teamStatsStyles.listItem,
                        fontWeight: moveType && store.pkmnType(pkmn).includes(moveType)
                          ? 500
                          : 400,
                      }}
                    >
