...
`src/app/footer/update-log/entries.ts` lists pokemon-related changes and features, such as "added Regulation M-C pokemon" or "Meganium can now learn Dazzling Gleam", plus serious UI bugs. Add an entry for those, newest first; never for small UI changes (showing an icon in an input), cosmetic fixes (a stray white box in dark mode), refactors, tooling, or other code-only changes.
